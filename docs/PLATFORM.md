...
## 부록 C — 구현 현황 (2026-08-07, 최종)

**Phase 0~3의 코드가 전부 들어갔고 운영 중인 사이트에 배포됐다.** 마이그레이션 0018~0023 적용 완료.

### C.0 실제로 돌아가는 것 (실측)

| 검증 | 결과 |
|---|---|
| 예약 제출 → DB → 좌석 증가 | ✅ 브라우저로 끝까지 완주, 점유석 0→1 |
| 토큰 링크 취소 → 좌석 반환 | ✅ 1→0, `cancelled_at` 기록 |
| 정원 초과 신청 | ✅ RPC가 거부 |
| 취소된 예약을 관리자가 승인 | ✅ 거부 (좌석 누수 없음) |
| anon 키로 `bookings` SELECT | ✅ 빈 배열 (PII 노출 0) |
| anon 키로 RPC 3종 직접 호출 | ✅ 404 |
| `/api/go` 모드 분기 | ✅ cooking→`/book/cooking`, hiking→구글폼 |
| 클릭 계측 | ✅ 진입점별 적재 시작 (구 DB에서 0건이던 것) |
...
명세만 보고는 안 보이던 세 곳이 화면을 켜 보니 드러났다. 셋 다 "모드를 바꿨는데 옛 경로가 남아 있는" 같은 종류다.

1. **허위 고지** — 클래스 상세의 개인정보 안내가 "Google Forms에서 접수"로 고정이었다. 자체 예약은 우리 DB가 직접 받는다. `PrivacyNote`가 mode를 받게 고침.
2. **인쇄물이 옛 목적지로** — QR·팝업이 전부 `/api/go`를 지나는데 native 전환 후에도 구글폼으로 나갔다. 종이는 회수할 수 없으니 이 라우트가 목적지를 책임진다. §13.2 분기표에 있었으나 구현이 빠져 있었다.
3. **QR 404** — 관리자가 새로 만든 자체 예약 체험은 구글폼 주소가 없어 QR이 안 나왔다.
...
**사람이 해야 하는 것 (코드로 안 되는 것)**

1. **관리자 Auth 계정 생성** — `admins` 행은 넣어 뒀다. Supabase 대시보드에서 계정만 만들면 `/admin`이 열린다 (docs/SETUP.md §2)
2. **도메인 구입** → `RESEND_API_KEY`·`BOOKING_FROM_EMAIL` 설정 → 예약 안내 메일 6종이 즉시 살아난다
3. **전화번호 확정** (U7) — `site_config.contact_phone`에 넣으면 사이트 전체에 자동 반영. 이메일 없는 시니어와 링크 분실자의 유일한 출구
4. **호스트 프로필 콘텐츠** — 김선영·조숙현 두 분이 이름·역할만 있고 소개글·사진·인용구가 비어 있다. `/admin/people`에서 채우면 상세 페이지가 살아난다
5. **취소 규정 최종 확인** (U11) — 현재 "시작 전까지 취소 가능"으로 약관과 RPC가 통일돼 있다

**코드로 남은 것 (v2.x)**

- `/privacy` 개정 (F17-3) — 예약이 실제로 운영되기 시작할 때. 수집 방법 "Google Forms"→"웹사이트 예약 폼", 이메일 발송 수탁자(Resend, 국외이전) 추가
- 팝업 `link_kind='class'` UI (0022는 적용됨, 관리자 화면 선택지 미노출)
- 리뷰·PG 결제·알림톡·대기자 명단 → Phase 4 게이트 (부록 B 행정 트랙 선행)
